...
export default function HouseCard({ house, onDelete }) {
  ...
  return (
    <article
      ...
      className="cursor-pointer overflow-hidden rounded-3xl border border-teal-100 bg-white/95 shadow-sm shadow-teal-900/5 transition hover:-translate-y-1 hover:shadow-md hover:shadow-teal-900/10 focus:outline-none focus:ring-2 focus:ring-teal-700 focus:ring-offset-2"
    >
      <div className="relative h-56">
        ...
        />
        <div className="absolute left-4 top-4 rounded-full bg-teal-900/95 px-3 py-1 text-sm text-white">
      ...
      <div className="p-5">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div>
            <h2 className="text-xl font-semibold text-teal-950">{house.title}</h2>
            <p className="text-sm text-teal-700">{house.location}</p>
          </div>
          <div className="flex flex-wrap justify-end gap-2">
            <span className="coast-pill">
              {house.type}
            </span>
            {pricePerSquareMeter && (
              <span className="rounded-full bg-stone-200 px-3 py-1 text-xs font-medium uppercase tracking-wide text-teal-950">
        ...
        <div className="flex flex-wrap gap-3" onClick={stopCardClick} onKeyDown={stopCardKeyDown}>
          ...
          <Link
            ...
            className="rounded-2xl border border-teal-200 bg-white px-4 py-2 text-sm text-teal-900 hover:bg-teal-50"
          >
            Edit
          </Link>
          <button
